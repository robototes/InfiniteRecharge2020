...
public class IndexerSensorSubsystem extends SubsystemBase{
    ...
    private DigitalInput m_intakeFrontSensor, m_indexFrontSensor, m_indexFrontMidSensor, m_indexMidSensor, m_indexBackMidSensor, m_indexBackSensor, m_intakeBackSensor;

    private DigitalInput[] m_sensorArray = {m_indexFrontSensor, m_indexFrontMidSensor, m_indexMidSensor, m_indexBackMidSensor, m_indexBackSensor};

    public IndexerSensorSubsystem(DigitalInput intakeFront, DigitalInput front, DigitalInput frontMid, DigitalInput mid, DigitalInput backMid, DigitalInput back, DigitalInput intakeBack) {
    ...
    public boolean allInnerSensorsOff(){
        if(!m_indexFrontSensor.get() && !m_indexFrontMidSensor.get() && !m_indexMidSensor.get() && !m_indexBackMidSensor.get() && !m_intakeBackSensor.get()){
            return true;
        } else{
            return false;
        }
