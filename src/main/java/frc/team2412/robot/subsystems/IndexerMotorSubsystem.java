...
public class IndexerMotorSubsystem extends SubsystemBase implements Loggable {
	...
	public IndexerMotorSubsystem(CANSparkMax frontMotor, CANSparkMax backMotor,
		...
		Trigger frontProcess = new Trigger(indexerSensorSubsystem::getIndexFrontSensorValue);
		frontProcess.whenActive(new IndexIntakeFrontCommandGroup(indexerSensorSubsystem, this), true);

		Trigger backProcess = new Trigger(indexerSensorSubsystem::getIndexBackSensorValue);
	...
	public void stopFrontPID(double val) {
		...
		// } else {
		// m_frontPIDController.setReference(frontTicks - (val *
		// IndexerConstants.INCH_STOP_DISTANCE),
		// ControlType.kPosition);
		// }
	}
	...
	public void stopBackPID(double val) {
		...
		m_backPIDController.setReference(backTicks + (val * IndexerConstants.INCH_STOP_DISTANCE),
				ControlType.kPosition);
		// } else {
		// m_backPIDController.setReference(backTicks - (val *
		// IndexerConstants.INCH_STOP_DISTANCE),
		// ControlType.kPosition);
		// }
	}
